...
export const userRegister = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    compiledHtml = compiledHtml({ firstName: firstName, email: email, url: `http://localhost:3000/auth/user/verification-user/${verificationCode}-TBX-${setTokenUser}` })

    await transporter.sendMail({
      to: email,
      subject: 'Verification your account for transaction',
      html: compiledHtml
    })
...
export const keepAuthUser = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    res.status(200).json({
      ...
      data: {
        isVerified: searchData[0]?.isVerified,
        ...
        identityNumber: searchData[0].identityNumber,
        refferalCode: searchData[0].referralCode,
      }
    })
...
export const getPointUser = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { userId } = req.body
...
export const forgotPassword = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    await prisma.$transaction(async (tx: any) => {
      ...
      const token = await encodeToken({ id: findUser[0].id, role: findUser[0].role })
      ...
      const emailHtml = fs.readFileSync('./src/public/emailSend/email.html', 'utf-8')
      let compiledHtml: any = await compile(emailHtml)
      compiledHtml = compiledHtml({ email: email, url: `http://localhost:3000/auth/user/forgot-password/${token}` })
