...
export const Header = () => {
  ...
  const handlePoints = async () => {
    try {
      const res = await instance.get('/auth/points-user')
  ...
  return (
    <>
      <nav className={`${pathname.startsWith('/event/dashboard') ? "hidden" : "block"} lg:px-20 w-full lg:py-2 fixed z-20`}>
        <section className="h-14 lg:h-20 top-0 px-5 items-center text-white justify-between flex lg:rounded-xl bg-blue-950">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="font-bold text-sm flex">
              <h1>Loyalty Points({!!token && points.length != 0 ? points[0].point : '0'})</h1>
          ...
          <div className="hidden lg:flex gap-5 items-center px-5">
            ...
            <div className="flex gap-3 ">
              {!!token ?
                ...
                :
                <>
                  <Link href={'/auth/user/register-user'}
                    className="py-2 hover:text-slate-300 border border-white px-5 rounded-xl transition-all duration-200 ease-in-out">
                    Register
                  </Link>
                  <Link href={'/auth/user/login-user'}
